...
#include "AtlasStyle.C"
  ...
  /// write a script that draws two histogram: one normal hist with colored line without color filling, one second hist draws as point
  /// write their in same
  /// make a ratio plot, that draws a ration of two histogram

  TH1F *DrawRatioPlot(TString TitleY, TH1F* hist1, TH1F* hist2, float setRangeA_Y = 0.51, float setRangeB_Y = 1.21, float setRangeA_X = 0., float setRangeB_X = 0.){
      ...
      float rel_uncert_from_1histo, rel_uncert_from_2histo, rel_uncert_overall;
      ...
      for(int i = 1; i<=nBins; i++){
        ...
        rel_uncert_overall = sqrt(pow(rel_uncert_from_1histo,2)+pow(rel_uncert_from_2histo,2));
  ...
  TH1F *DrawTwoHist(TString name, TString band1, TString band2, TH1F *hist1, TH1F *hist2, TString TitleX, TString TitleY, bool DrawPlot = false, bool SetNormalization = false, TString TitleY_sub = "ratio"){
      ...
      /// we do all settings from fist hist, and apply a settings of hist1 to hist1
     ...
     /// set parameters of the histograms (color, width ...)
      ...
      TLegend *leg1 = new TLegend(0.7042607,0.7278261,0.839599,0.8330435);
      ...
      leg1->AddEntry(hist1, Form("%s", band1.Data()),"p");
      leg1->AddEntry(hist2, Form("%s", band2.Data()),"lp");
      leg1->Draw();
      ...
      if (DrawPlot){
        ...
        DrawRatioPlot(TitleY_sub.Data(), hist1, hist2, -0.1, 2.1);
      }
